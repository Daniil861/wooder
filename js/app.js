...
da.init();
if (document.querySelector('.full__item_1')) {
	new Swiper('.full__item_1', {
		observer: true,
		observeParents: true,
		slidesPerView: 1,
		watchOverflow: true,
...
const menuLinks = document.querySelectorAll('.full__number[data-goto], .menu__link[data-goto], .full__mouse[data-goto]');
...
let menuContainer = document.querySelector('.header__container');
if (iconMenu != null) {
	let delay = 500;
...
// Управление блоком ланг
function openList() {
	let lang = document.querySelector('.header__lang');
	lang.classList.toggle('_active');
}
document.querySelector('.lang__arrow').addEventListener('click', openList);

//Убираем класс active при клике не по целевому блоку
...
// Добавляем класс checked выбранному языку

let langLinksEn = document.querySelector('.lang__link_en');
let langLinksRu = document.querySelector('.lang__link_ru');
langLinksEn.addEventListener('click', () => {
	if (langLinksRu.classList.contains('checked')) {
		langLinksRu.classList.remove('checked');
		langLinksEn.classList.remove('no-checked');
	}
	langLinksEn.classList.add('checked');
	langLinksRu.classList.add('no-checked');
	let lang = document.querySelector('.header__lang');
	lang.classList.remove('_active');


});
langLinksRu.addEventListener('click', () => {
	if (langLinksEn.classList.contains('checked')) {
		langLinksEn.classList.remove('checked');
		langLinksRu.classList.remove('no-checked');
	}
	langLinksRu.classList.add('checked');
	langLinksEn.classList.add('no-checked');
	let lang = document.querySelector('.header__lang');
	lang.classList.remove('_active');
})
		...
		for (let index = 0; index < scr_blocks.length; index++) {
			...
			*/
			...
			if ((pageYOffset > block_offset - window.innerHeight / 1.5) && pageYOffset < (block_offset + block_height) - window.innerHeight / 5) {
				...
				if (!block.classList.contains('_anim-no-hide')) {
